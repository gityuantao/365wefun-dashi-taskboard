...
- PR 不存在、目标分支不符或 head SHA 不符：门禁失败并退回「待开发」，评论精确原因。
- PR 合并冲突或保护规则拒绝：门禁失败并退回「待开发」，不强制绕过保护规则。
- 构建、部署或健康检查失败：退回「待开发」，记录失败阶段、非敏感错误和已有部署证据。
- 回读缺少 `gitSha`、返回 `local`、SHA 不匹配或不包含任务提交：退回「待开发」，绝不进入「待测试」。
- 下一轮开发作业必须读取最近一次 staging 失败反馈，将部署失败视为本轮首要返工项；修复后沿同一条“开发→验收→合并→部署→回读”链路重新执行。
- staging 失败沿用现有返工预算；连续失败达到预算上限后转「验收不通过」等待人工处理，避免永久循环消耗资源。
...
失败时发布“测试环境部署失败，已退回待开发”，附非敏感阶段、错误摘要、candidate 和 release id（若已产生）。失败反馈同时写入开发器下一轮可读取的上下文。不得使用原来的“开发完成（自动验收通过），进入待测试”文案冒充部署结果。
...
2. 未配置 staging adapter、PR 未合并、部署失败、`gitSha=local` 或 SHA 不匹配时，任务退回「待开发」，并留下下一轮开发可读取的失败原因。
...
7. staging 部署失败后，下一轮开发提示包含失败阶段和原因；修复完成后可以重新走完整门禁。连续失败达到既有返工预算时停止自动循环并转「验收不通过」。
