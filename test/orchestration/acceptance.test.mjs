...
test("acceptance waits for info when ClickUp comments cannot be fetched", async (t) => {
  const harness = await createCloudWorkerHarness();
  t.after(() => harness.dispose());
  await seedToAccepting(harness);
  const comments = [];
...
test("acceptance routes a Codex image decoder failure to waiting info", async (t) => {
  const harness = await createCloudWorkerHarness();
  t.after(() => harness.dispose());
  await seedToAccepting(harness);
  const comments = [];
  const result = await executeAcceptance({
    ...
    client: makeClient("version-9", {
      getComments: async () => [{
        id: "decoder-acceptance",
        images: [{ filename: "decoder-acceptance.png", url: "https://attachments.clickup.com/decoder-acceptance.png" }],
      }],
    ...
    codex: {
      run: async ({ imagePaths }) => ({
        exitCode: 1,
        stdout: "",
        stderr: `unsupported image decoder ${imagePaths[0]}?token=decoder-secret`,
      }),
    },
    now: NOW,
  });
  ...
  assert.equal(result.classification, "needs_info");
  assert.equal((await loadAggregate(harness.db, "task", "task-1")).state, "waiting_info");
  assert.match(comments[0], /comment image unavailable: decoder-acceptance\.png \(IMAGE_DECODE_FAILED\)/);
  assert.doesNotMatch(comments[0], /decoder-secret|taskboard-clickup-images-|\/tmp\//);
});

test("acceptance posts one safe ClickUp diagnostic when older comment images are truncated", async (t) => {
  const harness = await createCloudWorkerHarness();
  t.after(() => harness.dispose());
  await seedToAccepting(harness);
  const posts = [];
  const mediaComments = Array.from({ length: 9 }, (_, index) => ({
    id: `acceptance-limit-${index}`,
    date: String(9 - index),
    images: [{
  ...
  assert.match(diagnostics[0], /评论 acceptance-limit-8 图片：oldest\.png（IMAGE_LIMIT）/);
