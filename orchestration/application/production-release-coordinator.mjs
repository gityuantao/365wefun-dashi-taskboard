...
function releaseValues(target, response) {
  if (!response || typeof response !== "object") {
    throw validationFailure("production release adapter returned no submission evidence");
  }
  if (!nonEmpty(response.externalRequestId)) {
    throw validationFailure("production release adapter omitted externalRequestId");
  }
  if (target.platform === "ios" && !nonEmpty(response.buildNumber)) {
    throw validationFailure("iOS production release adapter omitted buildNumber");
  }
  return {
    ...
    observedEvidence: response.observedEvidence ?? response.evidence ?? target.observedEvidence,
  };
}
...
function webSuccessValues({ manifest, target, observed }) {
  ...
  return {
    ...
    readbackEvidence: observed.readbackEvidence,
  };
}

function iosSuccessValues({ app, target, observed }) {
  const buildNumber = observed?.buildNumber ?? target.buildNumber;
  const externalRequestId = observed?.externalRequestId ?? target.externalRequestId;
  const success = observed?.status === "completed"
    && nonEmpty(externalRequestId)
    && nonEmpty(buildNumber)
    && nonEmpty(observed?.uploadId ?? target.uploadId)
    && (observed?.processingStatus ?? target.processingStatus) === "processed"
    && nonEmpty(observed?.processingId ?? target.processingId)
    && (observed?.reviewStatus ?? target.reviewStatus) === "approved"
    && nonEmpty(observed?.reviewSubmissionId ?? target.reviewSubmissionId)
    ...
    && (observed?.liveMembershipConfirmed === true || observed?.liveEvidence != null)
    && observed?.observedEvidence != null;
...
function waitingValues(target, observed) {
  ...
  return {
    ...
    observedEvidence: observed?.observedEvidence ?? target.observedEvidence,
...
async function executeTarget({
  ...
  const requiresRecoveryReadback = target !== null
    ...
    try {
      submission = await fencedExternalEffect(
        ...
        (fencing) => adapter.release({
          ...
          platform: descriptor.platform,
          app: descriptor.app,
          idempotencyKey: `${manifest.versionId}:${manifest.checksum}:${descriptor.platform}:${descriptor.appId}:${target.attempt}`,
          recordStage,
          ...fencing,
        }),
      );
